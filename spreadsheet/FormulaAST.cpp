...
namespace ASTImpl {
    ...
    public:
        ...
        virtual double Evaluate(const std::function<double(Position)>& get_cell_pos_func) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const std::function<double(Position)>& get_cell_pos_func) const override {
                return get_cell_pos_func(*cell_);
            }
        ...
        public:
            ...
            double Evaluate([[maybe_unused]]const std::function<double(Position)>& get_cell_pos_func) const override {
                return value_;
            }
...
double FormulaAST::Execute(const std::function<double(Position)>& get_cell_pos_func) const {
    return root_expr_->Evaluate(get_cell_pos_func);
}
